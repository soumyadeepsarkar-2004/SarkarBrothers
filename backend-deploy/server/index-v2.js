...
app.patch('/api/admin/users/:id/role', authenticate, requireAdmin, auditLog('user_role_update'), async (req, res) => {
    try {
        const { role } = req.body;
        ...
        if (dbConnected) {
            const { User } = await import('./models.js');
            const user = await User.findByIdAndUpdate(req.params.id, { role, portalAccess }, { new: true }).select('-passwordHash').lean();
            if (user) return res.json(user);
        }
