...
export const createCustomerOrder = async ({ items, shippingAddress, user, dbConnected, isProd, stripe }) => {
    ...
    const mockShippingCost = orderTotal > 499 ? 0 : 100;
    orderTotal += mockShippingCost;
    ...
    if (dbConnected) {
        ...
        // Apply shipping logic: free over ₹499, else ₹100
        const shippingCost = verifiedTotal > 499 ? 0 : 100;
...
export const getAdminAnalytics = async ({ dbConnected, isProd }) => {
    if (dbConnected) {
        ...
        const lowStockProducts = await Product.find({ stock: { $lt: 10 } }).select('name stock category image').lean();
    ...
    return {
        ...
        data: {
            ...
            lowStockProducts: [
                { name: 'Mock Low Stock Toy', stock: 2 }
            ]
        }
    };
};
